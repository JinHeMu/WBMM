...
```text
tracer_jaka_description/urdf/tracer_jaka_zu5.urdf
        |
        +--> REMANI MMConfig
...
```bash
ros2 launch tracer_jaka_bringup ocs2_sim.launch.py use_csv_target:=false
```

or:

```bash
ros2 launch tracer_jaka_bringup ocs2_real.launch.py use_joy:=false
```
...
The NPZ coordinates are MuJoCo-world coordinates. The robot MJCF,
published odometry and generated scene ESDF now share the same origin, so the
integration launch defaults to `static_esdf_offset_x:=0.0`. Set the three
`static_esdf_offset_*` arguments only when importing an older scene/map that
uses an explicit fixed translation. Rotation between the MuJoCo and planner
axes is not supported by direct import and must be removed when generating the
grid.
...
geometry come directly from the canonical `tracer_jaka_zu5.urdf`. Both the front-end
