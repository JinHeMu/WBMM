...
机械臂: /arm_controller/commands (sim) 或 /jaka_forward_controller/commands (real)
```
...
| URDF | `urdf/tracer_jaka_zu5.urdf` | 仿真 URDF (ros2_control 用 gazebo_ros2_control) |
| URDF (实机) | `urdf/tracer_jaka_zu5_real.urdf` | 实机 URDF (ros2_control 用 jaka_hardware_interface) |
...
| 控制器 | `config/controllers.yaml` | ros2_control: base_controller + arm_controller |
...
| `urdf_file` | `tracer_jaka_zu5_real.urdf` | 机器人 URDF |
| `static_esdf_file` | `tracer_jaka_zu5_scene_esdf.npz` | 静态 ESDF |
| `static_esdf_offset_x/y/z` | `2.0/0.0/0.0` | ESDF 偏移 |
...
| jsb退出后 | jaka_forward_controller | 速度前向控制器 |
...
| `arm_cmd_topic` | `/jaka_forward_controller/commands` | 机械臂命令话题 |
...
### 6.4 实机控制器配置 (tracer_jaka_controllers_real.yaml)

| 控制器 | 类型 | 关节 |
|--------|------|------|
| `joint_state_broadcaster` | `joint_state_broadcaster/JointStateBroadcaster` | 所有关节 |
| `jaka_forward_controller` | `forward_command_controller/ForwardCommandController` | joint_1~joint_6 |
...
MuJoCo 场景中 `base_footprint` 固定在 world x=-2，而平面关节和 odom 从 x=0 开始，因此 `x_odom = x_mujoco + 2.0`。
...
# 或分步启动:
# 终端 1: 仅仿真 + MPC/MRT
ros2 launch tracer_jaka_ocs2 ocs2_sim.launch.py start_remani:=false start_slam:=false
# 终端 2: REMANI + 桥接
...
ros2 launch tracer_jaka_ocs2 ocs2_sim.launch.py start_slam:=false start_remani:=false
```
...
| `/jaka_forward_controller/commands` | `std_msgs/Float64MultiArray` | MRT→机械臂 | 机械臂速度命令 (实机) |
