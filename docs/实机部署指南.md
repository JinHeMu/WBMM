...
| 组件 | 当前工程位置 | 实机用途 |
| --- | --- | --- |
| Tracer CAN 驱动 | `src/drivers/base/tracer_base` | 订阅 `/cmd_vel`，发布底盘里程计 |
| JAKA ros2_control 接口 | `src/drivers/arm/jaka_hardware_interface` | EDG 状态读取和六关节位置命令 |
| 实机 OCS2 启动雏形 | `src/bringup/tracer_jaka_bringup/launch/ocs2_real.launch.py` | 启动底盘、JAKA 控制器、MPC/MRT |
| 实机定位 | `src/bringup/tracer_jaka_bringup/launch/real_slam.launch.py` | Tracer + IMU + Lakibeam + EKF + SLAM |
| 覆盖与力控 | `src/applications/wiping/wipe_planner` | 预接触、覆盖轨迹、导纳、力安全状态机 |
| 导航与全身规划 | `src/vendor/remani_planner/plan_manage` | 到达墙前预接触全身姿态 |
| 实机控制模型 | `src/algorithms/control/tracer_jaka_ocs2/config/task_real.info` | OCS2 实机参数入口 |
...
- [ ] 启动 `fts_broadcaster`，确认实际话题为 `/fts_broadcaster/wrench`。
...
flowchart LR
    ...
    FTS["六维力传感器"] --> WRENCH["/fts_broadcaster/wrench"]
    ...
    MRT --> CMDV["/cmd_vel"]
    MRT --> CMDQ["/arm_controller/commands"]
    CMDV --> BASE
    CMDQ --> ARM
```
...
- 配置中声明了 `fts_broadcaster`，但 launch 没有创建它的 spawner。
...
| 六维力 | `/fts_broadcaster/wrench` | `geometry_msgs/msg/WrenchStamped` | 100–125 Hz | F/T broadcaster |
...
| 机械臂命令 | `/arm_controller/commands` | `std_msgs/msg/Float64MultiArray` | 100–125 Hz | OCS2 MRT |
...
仿真、规划、MoveIt 与实机现在共用
`tracer_jaka_description/urdf/tracer_jaka_zu5.urdf`。硬件入口通过
`tracer_jaka_zu5.controlled.urdf.xacro` 的 `control_backend` 参数切换插件，不再复制
机械结构。自动测试会比较各后端的 `tool0`、关节限位、六球碰撞包络和传感器外参，
出现漂移时构建应失败。
...
2. 检查 `/fts_broadcaster/wrench` 的坐标系、频率和零点。
...
- [WipePlanner 说明](../src/applications/wiping/wipe_planner/README.md)
- [OCS2 说明](../src/algorithms/control/tracer_jaka_ocs2/README.md)
- [Tracer 驱动说明](../src/drivers/base/tracer_ros2/README.md)
