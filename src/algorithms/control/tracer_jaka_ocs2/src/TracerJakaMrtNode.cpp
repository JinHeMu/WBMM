...
public:
      ...
      : Node("tracer_jaka_mrt_node")
  {
    ...
    // Real-robot deployment safety gate.  When false the MRT still consumes
    // measurements, receives policies and publishes visualization, but it
    // does not even create command publishers for the base or arm.
    ...
    commandOutputEnabled_ =
        get_parameter("command_output_enabled").as_bool();
    ...
    if (!commandOutputEnabled_)
    {
      RCLCPP_WARN(
          get_logger(),
          "DRY-RUN safety gate active: base and arm command publishers "
          "are disabled. MPC/MRT computation and visualization remain active.");
    }
    else if (useStampedCmd_)
    {
  ...
  void initMrt()
  {
    // Launch-level __node remaps otherwise also rename this in-process helper
    // and make ros2 node list report several identical MRT node names.
    rclcpp::NodeOptions internalNodeOptions;
    internalNodeOptions.use_global_arguments(false);
    ocs2InternalNode_ =
