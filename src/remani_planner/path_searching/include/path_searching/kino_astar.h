...
class KinoAstar{
  private:
    ...
    rclcpp::Publisher<visualization_msgs::msg::Marker>::SharedPtr frontend_path_pub_;
    rclcpp::Publisher<visualization_msgs::msg::MarkerArray>::SharedPtr local_start_goal_pub_;
  ...
  public:
    ...
    KinoAstar(){}
    ~KinoAstar();
    void setParam(const rclcpp::Node::SharedPtr &node, const std::shared_ptr<GridMap> &env, const std::shared_ptr<MMConfig> &mm_config);
    // void init();
    void reset();
    ...
    void displayLocalStartGoal(const rclcpp::Publisher<visualization_msgs::msg::Marker>::SharedPtr &pub, const vector<Eigen::VectorXd> &list);
