...
: urdf_file_(urdf_file)
{
  ...
  if (surface["type"]) {
    surface_type_ = surface["type"].as<std::string>();
  }
  if (surface_type_ != "vertical" && surface_type_ != "horizontal") {
    throw std::runtime_error(
      "wipe_task.surface.type must be either 'vertical' or 'horizontal'");
  }
  ...
  if (surface["y_limits"]) {
    y_limits_ = yamlVector(surface["y_limits"]);
  } else {
    y_limits_ = Eigen::Vector2d(surface_center_.y(), surface_center_.y());
  }
  z_limits_ = yamlVector(surface["z_limits"]);
  if (surface_type_ == "horizontal" && std::abs(normal_.z()) < 0.90) {
    throw std::runtime_error(
      "A horizontal wipe surface requires a near-vertical normal_into_room");
  }

  const auto coverage = root["coverage"];
  if (coverage) {
    if (coverage["pattern"]) {
      coverage_pattern_ = coverage["pattern"].as<std::string>();
    }
    if (coverage["width"]) {
      ras_width_ = coverage["width"].as<double>();
    }
    if (coverage["height"]) {
      ras_height_ = coverage["height"].as<double>();
    }
  }
  if (coverage_pattern_ != "raster" && coverage_pattern_ != "ras") {
    throw std::runtime_error(
      "wipe_task.coverage.pattern must be either 'raster' or 'ras'");
  }
  if (ras_width_ <= 0.0 || ras_height_ <= 0.0) {
    throw std::runtime_error("RAS coverage width and height must be positive");
  }
  if (coverage_pattern_ == "ras" && surface_type_ != "vertical") {
    throw std::runtime_error(
      "RAS coverage is defined in the X-Z plane and requires a vertical surface");
  }
  // RAS width is measured along the wall direction (x_limits axis), so the
  // fit check projects onto wallDirection() instead of hardcoding odom X.
  const double ras_x_min = surface_center_.dot(wallDirection()) - 0.5 * ras_width_;
  const double ras_x_max = surface_center_.dot(wallDirection()) + 0.5 * ras_width_;
  const double ras_z_min = surface_center_.z() - 0.5 * ras_height_;
  const double ras_z_max = surface_center_.z() + 0.5 * ras_height_;
  if (coverage_pattern_ == "ras" &&
      (ras_x_min < x_limits_.x() - 1.0e-9 ||
       ras_x_max > x_limits_.y() + 1.0e-9 ||
       ras_z_min < z_limits_.x() - 1.0e-9 ||
       ras_z_max > z_limits_.y() + 1.0e-9))
  {
    throw std::runtime_error(
      "RAS coverage does not fit inside surface x_limits/z_limits");
  }
  ...
  if (whole_body["base_standoff_direction"]) {
    base_standoff_direction_ = yamlVector(
      whole_body["base_standoff_direction"]);
  } else {
    base_standoff_direction_ = Eigen::Vector3d(normal_.x(), normal_.y(), 0.0);
  }
  base_standoff_direction_.z() = 0.0;
  if (base_standoff_direction_.head<2>().norm() < 1.0e-6) {
    throw std::runtime_error(
      "whole_body.base_standoff_direction must define the table-side base direction");
  }
...
Eigen::Vector3d Planner::wallDirection() const
{
  // For a vertical surface the wall runs horizontally, perpendicular to the
  // surface normal: cross(Z, n) = (-n_y, n_x, 0). The sign is chosen so that
  // the x_limits midpoint lies on the +wall_dir side of the center projection,
  // which makes x_limits natural coordinates along the wall (for a normal
  // Y-plane wall this reduces exactly to odom X). A horizontal surface
  // returns +X; rasterTargets never uses it in that branch.
  Eigen::Vector3d wall_dir(-normal_.y(), normal_.x(), 0.0);
  if (wall_dir.squaredNorm() < 1.0e-12) {
    return Eigen::Vector3d::UnitX();
  }
  wall_dir.normalize();
  const double limits_mid = 0.5 * (x_limits_.x() + x_limits_.y());
  if (limits_mid > surface_center_.dot(wall_dir)) {
    wall_dir = -wall_dir;
  }
  return wall_dir;
}
...
std::vector<Eigen::Vector3d> Planner::rasterTargets() const
{
  ...
  const Eigen::Vector3d wall_dir = wallDirection();
  for (int row = 0; row < raster_rows_; ++row) {
    const double row_ratio = raster_rows_ == 1 ?
      (surface_type_ == "horizontal" ? 0.5 : 0.0) :
    ...
    for (int column = 0; column < raster_columns_; ++column) {
      ...
      if (surface_type_ == "horizontal") {
        point.x() = x_limits_.x() + x_ratio * (x_limits_.y() - x_limits_.x());
        point.y() = y_limits_.x() + row_ratio * (y_limits_.y() - y_limits_.x());
      } else {
        // Vertical: x_limits are coordinates along the wall direction. The
        // wall-parallel offset keeps the whole stroke on the surface plane.
        const double x_local = x_limits_.x() + x_ratio * (x_limits_.y() - x_limits_.x());
        point += wall_dir * (x_local - surface_center_.dot(wall_dir));
        point.z() = z_limits_.x() + row_ratio * (z_limits_.y() - z_limits_.x());
      }
      coarse.push_back(point);
    }
  }
  return densifyTargets(coarse);
}

std::vector<Eigen::Vector3d> Planner::rasTargets() const
{
  // Normalized, single-stroke R-A-S centerline. R and S use cubic Bezier
  // samples for rounded turns. The A crossbar is intentionally retraced once:
  // this keeps a recognizable crossbar without lifting the tool or inserting
  // a non-contact jump between letters.
  std::vector<Eigen::Vector2d> normalized;
  normalized.emplace_back(0.00, 0.00);
  auto lineTo = [&normalized](double u, double v) {
      normalized.emplace_back(u, v);
    };
  auto cubicTo = [&normalized](const Eigen::Vector2d & control_first,
                               const Eigen::Vector2d & control_second,
                               const Eigen::Vector2d & end) {
      const Eigen::Vector2d start = normalized.back();
      constexpr int kCurveSamples = 12;
      for (int sample = 1; sample <= kCurveSamples; ++sample) {
        const double t = static_cast<double>(sample) / kCurveSamples;
        const double one_minus_t = 1.0 - t;
        normalized.push_back(
          one_minus_t * one_minus_t * one_minus_t * start +
          3.0 * one_minus_t * one_minus_t * t * control_first +
          3.0 * one_minus_t * t * t * control_second +
          t * t * t * end);
      }
    };

  // R: lower-left -> stem -> rounded bowl -> middle -> diagonal leg.
  lineTo(0.00, 1.00);
  lineTo(0.18, 1.00);
  cubicTo(Eigen::Vector2d(0.31, 1.00), Eigen::Vector2d(0.31, 0.52),
    Eigen::Vector2d(0.18, 0.52));
  lineTo(0.00, 0.52);
  lineTo(0.30, 0.00);

  // A: R's diagonal ends at the left foot. Draw the apex, right side,
  // crossbar (out-and-back), then finish at the right foot.
  lineTo(0.47, 1.00);
  lineTo(0.55, 0.42);
  lineTo(0.39, 0.42);
  lineTo(0.55, 0.42);
  ...
  // S, drawn bottom-to-top so it continues directly from the A right foot.
  cubicTo(Eigen::Vector2d(0.88, 0.00), Eigen::Vector2d(1.00, 0.04),
    Eigen::Vector2d(1.00, 0.23));
  cubicTo(Eigen::Vector2d(1.00, 0.44), Eigen::Vector2d(0.83, 0.49),
    Eigen::Vector2d(0.71, 0.52));
  cubicTo(Eigen::Vector2d(0.63, 0.59), Eigen::Vector2d(0.64, 0.76),
    Eigen::Vector2d(0.73, 0.81));
  cubicTo(Eigen::Vector2d(0.80, 0.94), Eigen::Vector2d(0.92, 1.00),
    Eigen::Vector2d(1.00, 1.00));

  const Eigen::Vector3d plane = surface_center_ - contact_offset_ * normal_;
  const Eigen::Vector3d wall_dir = wallDirection();
  std::vector<Eigen::Vector3d> coarse;
  coarse.reserve(normalized.size());
  for (const auto & point : normalized) {
    // (u, v) -> wall-direction x ras_width, odom Z x ras_height, on the plane.
    Eigen::Vector3d target = plane;
    target += wall_dir * ((point.x() - 0.5) * ras_width_);
    target.z() = target.z() + (point.y() - 0.5) * ras_height_;
    coarse.push_back(target);
  }
  return densifyTargets(coarse);
}

std::vector<Eigen::Vector3d> Planner::densifyTargets(
...
std::vector<Eigen::Vector3d> Planner::coverageTargets() const
{
  return coverage_pattern_ == "ras" ? rasTargets() : rasterTargets();
}

Eigen::Vector3d Planner::precontactTarget() const
{
  const auto targets = coverageTargets();
  if (targets.empty()) {
    throw std::runtime_error("Cannot form a pre-contact target without coverage points");
  }
  return targets.front() + approach_clearance_ * normal_;
}
  ...
  const Eigen::Vector3d & target) const
{
  ...
  const Eigen::Vector2d standoff_direction =
    base_standoff_direction_.head<2>();
  const Eigen::Vector2d tangent(
    -standoff_direction.y(), standoff_direction.x());
  const Eigen::Vector2d surface_center = surface_center_.head<2>();
  const double target_longitudinal =
    tangent.dot(target.head<2>() - surface_center);
       ...
       standoff += std::max(1.0e-3, candidate_lateral_step_))
  {
         ...
         lag += std::max(1.0e-3, candidate_longitudinal_step_))
    {
      ...
      {
        const Eigen::Vector2d base_position = surface_center +
          standoff * standoff_direction +
          (target_longitudinal + lag) * tangent;
        candidates.emplace_back(base_position.x(), base_position.y(),
...
std::vector<Eigen::VectorXd> Planner::precontactCandidates() const
{
  const auto targets = coverageTargets();
  if (targets.empty()) {
    return {};
  ...
  std::vector<Eigen::Vector3d> entry_bases;
  // The online task intentionally exposes only one deterministic REMANI
  // handoff pose.  The offline future-task experiment instead samples the
  // admissible local whole-body manifold without changing that online config.
  // Every sampled base remains inside the same standoff tube subsequently
  // enforced by Hybrid A*.
  const double standoff_lower =
    standoff_limits_.x() - 0.95 * hybrid_standoff_margin_;
  const double standoff_upper =
    standoff_limits_.y() + 0.95 * hybrid_standoff_margin_;
  const Eigen::Vector2d standoff_direction =
    base_standoff_direction_.head<2>();
  const Eigen::Vector2d tangent(
    -standoff_direction.y(), standoff_direction.x());
  const Eigen::Vector2d surface_center = surface_center_.head<2>();
  const double target_longitudinal =
    tangent.dot(targets.front().head<2>() - surface_center);
  constexpr int kLongitudinalSamples = 13;
  constexpr int kStandoffSamples = 5;
  constexpr int kYawSamples = 7;
  for (int longitudinal = 0; longitudinal < kLongitudinalSamples; ++longitudinal) {
    const double longitudinal_ratio = static_cast<double>(longitudinal) /
      static_cast<double>(kLongitudinalSamples - 1);
    const double lag = -0.24 + 0.48 * longitudinal_ratio;
    for (int standoff_index = 0; standoff_index < kStandoffSamples;
         ++standoff_index)
    {
      const double standoff_ratio = static_cast<double>(standoff_index) /
        static_cast<double>(kStandoffSamples - 1);
      const double standoff = standoff_lower +
        standoff_ratio * (standoff_upper - standoff_lower);
      for (int yaw_index = 0; yaw_index < kYawSamples; ++yaw_index) {
        const double yaw_ratio = static_cast<double>(yaw_index) /
          static_cast<double>(kYawSamples - 1);
        const double yaw = wrapAngle(cleaning_yaw_ - 0.15 + 0.30 * yaw_ratio);
        const Eigen::Vector2d base_position = surface_center +
          standoff * standoff_direction +
          (target_longitudinal + lag) * tangent;
        entry_bases.emplace_back(base_position.x(), base_position.y(), yaw);
      }
    }
  }
  std::vector<Eigen::VectorXd> result;
  for (const Eigen::Vector3d & base : entry_bases) {
    for (const Eigen::VectorXd * seed : {&nominal_ik_seed_, &search_ik_seed_}) {
      IkResult ik = solveIk(base, targets.front(), *seed);
      if (!ik.success || !armCollisionFree(ik.q)) {
        continue;
      }
      bool valid = true;
      Eigen::VectorXd previous = ik.q;
      for (int step = 1; step <= steps; ++step) {
        const double distance = approach_clearance_ *
          static_cast<double>(step) / static_cast<double>(steps);
        ik = solveIk(base, targets.front() + distance * normal_, previous);
        if (!ik.success || !armCollisionFree(ik.q) ||
            !armMotionCollisionFree(previous, ik.q))
        {
          valid = false;
          break;
        }
      ...
      const bool duplicate = std::any_of(
        result.begin(), result.end(), [&state](const Eigen::VectorXd & existing) {
          return (existing - state).norm() < 1.0e-5;
        });
      if (!duplicate) {
                            ...
                            const Eigen::Vector2d & upper) const
{
  ...
  const double standoff = base_standoff_direction_.head<2>().dot(
    base.head<2>() - surface_center_.head<2>());
...
bool Planner::isArmConfigurationCollisionFree(const Eigen::VectorXd & q) const
{
  return q.size() == model_.nq && armCollisionFree(q);
}
  ...
  const Eigen::Vector3d & goal_target, PlanReport & report) const
{
  ...
  auto finish = [&](int parent_index) {
      ...
      // The node itself is produced by an exact differential-drive primitive.
      // Finish at the exact goal base only when the remaining motion is a
      // valid straight unicycle segment (same yaw, no lateral component):
      // the snap absorbs the primitive quantization (a node can land up to
      // ~1.5 cm from the sampled goal at the 3 cm primitive length) while
      // keeping every terminal state on the candidate grid, so the next
      // segment starts from an exact candidate pose again.  For oblique goals
      // the linear snap is not a valid differential-drive motion; finish at
      // the reached node and let the collision-checked fixed-base arm motion
      // hit the exact tool pose.  A colliding snap falls back to the node
      // finish.
      const Eigen::Vector3d base_delta = goal_base - parent.base;
      const double straight_lateral = std::abs(
        -std::sin(parent.base.z()) * base_delta.x() +
        std::cos(parent.base.z()) * base_delta.y());
      const bool exact_straight_finish =
        std::abs(wrapAngle(goal_base.z() - parent.base.z())) <= 1.0e-6 &&
        straight_lateral <= 1.0e-6;
      const Eigen::Vector3d node_base = parent.base;
      IkResult final_ik = solveIk(node_base, goal_target, parent.q);
      bool use_snap = false;
      if (exact_straight_finish) {
        const IkResult snap_ik = solveIk(goal_base, goal_target, parent.q);
        use_snap = snap_ik.success &&
          wholeBodyMotionCollisionFree(
            parent.base, parent.q, goal_base, snap_ik.q, lower, upper);
        if (use_snap) {
          final_ik = snap_ik;
        }
      }
      ...
      const Eigen::Vector3d terminal_base = use_snap ? goal_base : node_base;
      // Straight-line joint motion between adjacent contact poses can sweep
      // the forearm through the base protection spheres even when both
      // endpoint poses are clear; reject the segment and let the search keep
      // looking for a chain whose arm motion is clear.
      if (!armMotionCollisionFree(parent.q, final_ik.q)) {
      ...
      if (result.bases.empty() ||
          (result.bases.back() - terminal_base).norm() > 1.0e-8)
      {
        result.bases.push_back(terminal_base);
        result.joints.push_back(final_ik.q);
        result.contact_targets.push_back(goal_target);
      } else {
        result.bases.back() = terminal_base;
      ...
      result.cost = parent.g + hybrid_joint_motion_penalty_ *
        (final_ik.q - parent.q).norm();
      result.success = true;
      return result;
    };

  // Adjacent coverage samples are only a few centimetres apart.  First keep
  // the base at the last exact differential-drive state and let the arm reach
  // the next tool pose through a collision-checked joint motion.  Moving the
  // base for every Cartesian sample creates unnecessary steering and, for an
  // oblique pre-contact pose, can introduce a nonholonomic terminal snap.
  // Hybrid A* is still used as soon as the fixed-base arm motion is infeasible.
  ++report.hybrid_expanded_nodes;
  SearchResult fixed_base = finish(0);
  if (fixed_base.success) {
    return fixed_base;
  }
...
Planner::BaseCandidate Planner::continueWholeBodyPath(
  const std::vector<Eigen::Vector3d> & targets,
  const Eigen::Vector3d & start_base, const Eigen::VectorXd & start_q,
  PlanReport & report) const
{
  if (targets.empty() || start_q.size() != model_.nq) {
    throw std::runtime_error("Pre-contact continuation received invalid input");
  }
  if (!armCollisionFree(start_q)) {
    throw std::runtime_error("Pre-contact approach reached a self-colliding contact state");
  }

  BaseCandidate result;
  result.cost = 0.0;
  result.bases = {start_base};
  result.joints = {start_q};
  result.contact_targets = {targets.front()};
  for (std::size_t target_index = 1; target_index < targets.size(); ++target_index) {
    SearchResult best_segment;
    for (const auto & goal_base : reachableBaseCandidates(targets[target_index])) {
      SearchResult segment = hybridAstarSegment(
    ...
    if (!best_segment.success) {
      throw std::runtime_error(
        "Pre-contact-conditioned Hybrid A* failed at coverage keypoint " +
        std::to_string(target_index) + "/" +
        std::to_string(targets.size() - 1));
    }
    result.cost += best_segment.cost;
  ...
  for (std::size_t i = 0; i < candidate.bases.size(); ++i) {
    if (i > 0) {
      ...
      const bool row_change =
        std::abs(candidate.contact_targets[i].y() -
          candidate.contact_targets[i - 1].y()) > 1.0e-6 ||
        std::abs(candidate.contact_targets[i].z() -
  ...
  PlanReport & report) const
{
  ...
  // execution from the free side towards the contact surface.
  ...
  for (int step = 1; step <= steps; ++step) {
    ...
    if (!ik.success || !armCollisionFree(ik.q)) {
      throw std::runtime_error(
        "No collision-free surface-normal pre-contact IK at clearance " +
  ...
  for (int index = steps; index >= 1; --index) {
    ...
    // tool already surface-normal before any motion towards contact begins.
...
std::vector<Waypoint> Planner::planFromPrecontact(
  const Eigen::VectorXd & precontact_state, PlanReport & report) const
{
  if (precontact_state.size() != model_.nq + 3) {
    throw std::runtime_error("Pre-contact state has the wrong state dimension");
  }
  const auto targets = coverageTargets();
  if (targets.empty()) {
    throw std::runtime_error("Pre-contact rollout has no coverage targets");
  }
  const Eigen::Vector3d base = precontact_state.head<3>();
  const Eigen::VectorXd entry_q = precontact_state.tail(model_.nq);
  if (!armCollisionFree(entry_q)) {
    throw std::runtime_error("Input pre-contact state is self-colliding");
  }

  Eigen::Vector3d entry_position;
  Eigen::Matrix3d entry_rotation;
  forward(base, entry_q, entry_position, entry_rotation);
  const Eigen::Vector3d expected_entry =
    targets.front() + approach_clearance_ * normal_;
  const double entry_position_error = (entry_position - expected_entry).norm();
  const Eigen::Matrix3d entry_rotation_error =
    desired_tool_rotation_.transpose() * entry_rotation;
  const double entry_axis_error = std::acos(std::clamp(
    0.5 * (entry_rotation_error.trace() - 1.0), -1.0, 1.0));
  if (entry_position_error > ik_position_tolerance_ ||
      entry_axis_error > ik_axis_tolerance_)
  {
    throw std::runtime_error(
      "Input state does not satisfy the configured pre-contact tool pose "
      "(position error=" + std::to_string(entry_position_error) +
      " m, axis error=" + std::to_string(entry_axis_error) + " rad)");
  }
  report.max_position_error = entry_position_error;
  report.max_axis_error = entry_axis_error;

  const int steps = std::max(2, static_cast<int>(std::ceil(
    approach_clearance_ / std::max(0.01, solver_spacing_))));
  const double cartesian_segment_length =
    approach_clearance_ / static_cast<double>(steps);
  std::vector<Waypoint> result;
  result.reserve(static_cast<std::size_t>(steps + 2));
  Waypoint entry;
  entry.time = 0.0;
  entry.state = precontact_state;
  entry.input = Eigen::VectorXd::Zero(model_.nq + 2);
  entry.contact_target = expected_entry;
  entry.in_contact = false;
  result.push_back(entry);

  double elapsed = 0.0;
  if (precontact_hold_duration_ > 1.0e-6) {
    Waypoint hold = entry;
    hold.time = precontact_hold_duration_;
    result.push_back(std::move(hold));
    elapsed = precontact_hold_duration_;
  }

  Eigen::VectorXd previous = entry_q;
  Eigen::VectorXd contact_q;
  for (int step = steps - 1; step >= 0; --step) {
    const double distance = approach_clearance_ *
      static_cast<double>(step) / static_cast<double>(steps);
    const Eigen::Vector3d target = targets.front() + distance * normal_;
    const IkResult ik = solveIk(base, target, previous);
    if (!ik.success || !armCollisionFree(ik.q) ||
        !armMotionCollisionFree(previous, ik.q))
    {
      throw std::runtime_error(
        "Direct surface-normal approach failed at clearance " +
        std::to_string(distance) + " m");
    }
    const bool touch_segment = distance < approach_touch_distance_ - 1.0e-9;
    const double speed = touch_segment ? approach_touch_speed_ : approach_speed_;
    const double joint_duration =
      (ik.q - previous).cwiseAbs().maxCoeff() /
      std::max(1.0e-3, max_joint_speed_);
    elapsed += std::max(cartesian_segment_length / speed, joint_duration);
    report.max_position_error = std::max(report.max_position_error, ik.position_error);
    report.max_axis_error = std::max(report.max_axis_error, ik.axis_error);
    previous = ik.q;
    if (step == 0) {
    ...
    approach.state.resize(model_.nq + 3);
    approach.state << base, ik.q;
    approach.input = Eigen::VectorXd::Zero(model_.nq + 2);
    approach.contact_target = target;
    approach.in_contact = false;
    result.push_back(std::move(approach));
  }

  const BaseCandidate candidate = continueWholeBodyPath(
    targets, base, contact_q, report);
  auto contact = buildContactTrajectory(targets, candidate, report);
  for (Waypoint & waypoint : contact) {
    waypoint.time += elapsed;
    result.push_back(std::move(waypoint));
  }
  fillFeedforward(result, report);
  if (!result.front().state.isApprox(precontact_state, 1.0e-12)) {
    throw std::runtime_error("Pre-contact rollout changed its prescribed first frame");
  }
  report.points = result.size();
  report.duration = result.empty() ? 0.0 : result.back().time;
  return result;
...
Eigen::VectorXd Planner::normalOffsetCorrectedState(const Eigen::VectorXd & state,
...
TrajectoryMetrics Planner::evaluateTrajectory(
  const std::vector<Waypoint> & trajectory) const
{
  if (trajectory.empty()) {
    throw std::runtime_error("Cannot evaluate an empty trajectory");
  }

  TrajectoryMetrics metrics;
  pinocchio::Data data(model_);
  pinocchio::GeometryData geometry_data(collision_model_);
  for (const Waypoint & waypoint : trajectory) {
    if (waypoint.state.size() != model_.nq + 3 ||
        waypoint.input.size() != model_.nv + 2)
    {
      throw std::runtime_error(
        "Trajectory quality received a waypoint with the wrong dimension");
    }
    const Eigen::VectorXd q = waypoint.state.tail(model_.nq);
    const Eigen::VectorXd lower_margin = q - model_.lowerPositionLimit;
    const Eigen::VectorXd upper_margin = model_.upperPositionLimit - q;
    const Eigen::VectorXd range =
      (model_.upperPositionLimit - model_.lowerPositionLimit)
      .cwiseMax(1.0e-9);
    metrics.min_joint_margin = std::min(
      metrics.min_joint_margin,
      std::min(lower_margin.minCoeff(), upper_margin.minCoeff()));
    metrics.min_normalized_joint_margin = std::min(
      metrics.min_normalized_joint_margin,
      std::min(
        lower_margin.cwiseQuotient(range).minCoeff(),
        upper_margin.cwiseQuotient(range).minCoeff()));
    ...
    for (Eigen::Index index = 0; index < svd.singularValues().size(); ++index) {
      manipulability *= std::max(0.0, svd.singularValues()[index]);
    }
    metrics.min_manipulability = std::min(
      metrics.min_manipulability, manipulability);
    ...
    if (!collision_model_.collisionPairs.empty()) {
      ...
      if (nearest < geometry_data.distanceResults.size()) {
        metrics.min_self_collision_clearance = std::min(
          metrics.min_self_collision_clearance,
          geometry_data.distanceResults[nearest].min_distance);
      }
    }
    metrics.max_joint_speed = std::max(
      metrics.max_joint_speed,
      waypoint.input.tail(model_.nv).cwiseAbs().maxCoeff());
