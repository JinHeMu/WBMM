...
public:
  ...
  {
    declareParameters();
    planner_frame_ = get_parameter("planner_frame").as_string();
    ...
    if (planner_frame_.empty()) {
      planner_frame_ = world_frame;
    }
    if (world_frame != planner_frame_) {
      throw std::runtime_error(
        "world_frame and planner_frame must match: all wipe geometry is "
        "planned and visualized in one persistent frame");
    }
    if (planner_frame_.empty() || control_frame_.empty()) {
      throw std::runtime_error("planner_frame and control_frame cannot be empty");
    }
    ...
    collision_fault_subscription_ = create_subscription<std_msgs::msg::Bool>(
      get_parameter("collision_fault_topic").as_string(), rclcpp::QoS(10).reliable(),
      std::bind(&WipePlannerNode::collisionFaultCallback, this, std::placeholders::_1));
    ...
    whole_body_trajectory_publisher_ =
      create_publisher<trajectory_msgs::msg::JointTrajectory>(
      "/wipe_planner/whole_body_trajectory", transient);
    phase_schedule_publisher_ = create_publisher<std_msgs::msg::String>(
      "/wipe_planner/phase_schedule", transient);
    live_state_publisher_ = create_publisher<sensor_msgs::msg::JointState>(
      "/wipe_planner/live_state", rclcpp::QoS(1).reliable());
    ...
    RCLCPP_INFO(get_logger(),
      ...
      "planner_frame=%s, control_frame=%s, tf=%s",
      force_control_enabled_ ? "enabled" : "disabled",
      planner_frame_.c_str(), control_frame_.c_str(),
      use_tf_transform_ ? "enabled" : "disabled");
    RCLCPP_INFO(
      get_logger(),
      "Pre-contact handoff: complete 9D squared error <= %.4f for %.2f s; "
      "base/yaw-only fallback=%s",
      get_parameter("navigation_whole_body_squared_tolerance").as_double(),
      get_parameter("navigation_arrival_hold").as_double(),
      get_parameter("navigation_partial_handoff_enabled").as_bool() ?
      "enabled" : "disabled");
  }
...
private:
  enum class Phase {WAITING, NAVIGATING, PLANNING, WIPING, COMPLETED, FAILED};

  void declareParameters()
  {
    declare_parameter("robot_name", "mobile_manipulator");
    declare_parameter("trajectory_topic", "/planning/trajectory");
    declare_parameter("wrench_topic", "/fts_broadcaster/wrench");
    declare_parameter("collision_fault_enabled", true);
    declare_parameter("collision_fault_topic", "/mujoco/unexpected_collision");
    declare_parameter("remani_task_service", "/remani_planner/set_task_execution");
    declare_parameter("world_frame", "odom");
    declare_parameter("planner_frame", "odom");
    declare_parameter("control_frame", "odom");
    declare_parameter("use_tf_transform", false);
    declare_parameter("tf_lookup_timeout", 0.10);
    ...
    declare_parameter("navigation_partial_handoff_enabled", true);
    ...
    declare_parameter("task_completion_hold", 0.50);
    declare_parameter("task_completion_base_tolerance", 0.02);
    declare_parameter("task_completion_joint_tolerance", 0.02);
    declare_parameter("task_completion_ee_tolerance", 0.015);
  ...
  void collisionFaultCallback(const std_msgs::msg::Bool::SharedPtr message)
  {
    if (!message->data ||
        !get_parameter("collision_fault_enabled").as_bool())
    {
      return;
    }
    if (collision_fault_latched_.exchange(true)) {
      return;
    }
    ...
    RCLCPP_ERROR(
      get_logger(),
      "MuJoCo reported an unexpected physical collision; latching FAILED "
      "and publishing a measured-state hold reference");
  }
    ...
    const std::string & target_frame, const std::string & source_frame)
  {
    ...
    if (!use_tf_transform_) {
      RCLCPP_ERROR_THROTTLE(
        ...
        "Frame conversion %s <- %s is required but use_tf_transform is false",
    ...
    try {
      const auto transform = tf_buffer_->lookupTransform(
        ...
        tf2::durationFromSec(std::max(
          0.0, get_parameter("tf_lookup_timeout").as_double())));
      const auto & quaternion = transform.transform.rotation;
      PlanarTransform planar;
      planar.x = transform.transform.translation.x;
      planar.y = transform.transform.translation.y;
      planar.yaw = std::atan2(
      ...
      RCLCPP_WARN_THROTTLE(
        get_logger(), *get_clock(), 3000,
        "Waiting for required TF %s <- %s: %s; no MPC reference is published",
  ...
  void observationCallback(const ocs2_msgs::msg::MpcObservation::SharedPtr message)
  {
    ...
    Eigen::VectorXd planner_state = toEigen(message->state.value);
    if (planner_frame_ != control_frame_) {
      const auto planner_from_control = lookupPlanarTransform(
        planner_frame_, control_frame_);
      if (!planner_from_control) {
        return;
      }
      planner_state = transformWholeBodyState(
        planner_state, *planner_from_control);
    }
    auto planner_observation = *message;
    planner_observation.state.value = toFloatVector(planner_state);
    std::lock_guard<std::mutex> lock(mutex_);
    observation_ = std::move(planner_observation);
  ...
  void wrenchCallback(const geometry_msgs::msg::WrenchStamped::SharedPtr message)
  {
    ...
    double filtered_force = 0.0;
    {
      ...
      if (forceContactActiveLocked() && !force_hard_stop_) {
        ...
        // missing contact force must not accumulate maximum surface penetration
  ...
  void startCoveragePreview(Eigen::VectorXd seed)
  {
    ...
    planning_thread_ = std::thread([this, seed = std::move(seed)]() {
      try {
        // The planner returns a surface-normal pre-contact prefix followed by the
        ...
        // A contact-valid posture is not necessarily a valid pre-contact
        // posture: lifting the tool can fold the elbow into the base. Generate
        // the explicit pre-contact manifold first, then retain the nearest
        // complete rollout whose normal approach and coverage both pass the
        // URDF collision checks. This 9D state is the only goal sent to REMANI.
        auto candidates = planner_->precontactCandidates();
        if (candidates.empty()) {
          throw std::runtime_error(
            "No collision-free complete 9D pre-contact candidate exists");
        }
        std::sort(candidates.begin(), candidates.end(),
          [&seed](const Eigen::VectorXd & first, const Eigen::VectorXd & second) {
            const auto cost = [&seed](const Eigen::VectorXd & state) {
                const double base = (state.head<2>() - seed.head<2>()).norm();
                const double yaw = std::abs(wrapAngle(state[2] - seed[2]));
                const double arm =
                  (state.tail<6>() - seed.tail<6>()).cwiseAbs().maxCoeff();
                return base + 0.25 * yaw + 0.15 * arm;
              };
            return cost(first) < cost(second);
          });
        PlanReport report;
        std::vector<Waypoint> preview;
        std::string last_error;
        for (const Eigen::VectorXd & candidate : candidates) {
          try {
            PlanReport candidate_report;
            auto candidate_preview = planner_->planFromPrecontact(
              candidate, candidate_report);
            preview = std::move(candidate_preview);
        ...
        if (preview.empty()) {
          throw std::runtime_error(
            "All collision-free pre-contact candidates failed the complete "
            "task rollout; last error: " + last_error);
        }
        if (preview.empty()) {
        ...
        // The full coverage visualization is deliberately deferred to the
        // pre-contact handoff (startPlanning) so RViz shows nothing wipe-like
        // while REMANI is still navigating.
        ...
        if (get_parameter("auto_navigation_goal").as_bool() &&
            !shutting_down_.load() && !collision_fault_latched_.load())
        {
          ...
          RCLCPP_INFO(get_logger(),
            "Sent surface-normal pre-contact state (base + 6 joints) to REMANI on %s",
  ...
  void startPlanning(Eigen::VectorXd seed)
  {
    ...
    planning_thread_ = std::thread([this, seed = std::move(seed)]() {
      try {
        ...
        RCLCPP_INFO(get_logger(),
          "Executing base-hold arm alignment, surface-normal approach, then contact "
          "coverage (handoff max state error %.3f)", initial_error);
        if (shutting_down_.load()) {
          return;
        }
        publishVisualization(trajectory);
        publishWholeBodyContract(trajectory);
        {
          std::lock_guard<std::mutex> lock(mutex_);
          ...
          force_safety_anchor_.reset();
          resetForceSafetyRetreatLocked();
          if (collision_fault_latched_.load()) {
            throw std::runtime_error(
              "MuJoCo collision fault latched while preparing task execution");
          }
          phase_ = Phase::WIPING;
        }
        RCLCPP_INFO(get_logger(),
  ...
  Eigen::VectorXd forceCorrectedReferenceLocked(const Eigen::VectorXd & state) const
  {
    ...
    Eigen::VectorXd corrected = planner_->normalOffsetCorrectedState(
      state, total_offset, max_joint_correction);

    // Before the first force sample, never let the OCS2 reference get far
    // ahead of the measured tool along the surface normal.  The admittance keeps
    ...
    // contact reference propagates through MPC, producing a stiff-surface impact.
    if (!force_contact_detected_ && observation_) {
      ...
      if (normal_lead < -max_wallward_lead) {
        ...
        corrected = planner_->normalOffsetCorrectedState(
          state, total_offset, max_joint_correction);
      }
    }
    return corrected;
    ...
    double sample_time, double yaw_reference) const
  {
    ...
    if (forceSafetyStopLocked() && forceContactActiveLocked()) {
      ...
      retreat = planner_->normalOffsetCorrectedState(
  ...
  void updateVirtualProgressLocked(double current_time)
  {
    ...
    if (contact_phase && !contact_settle_phase) {
      ...
      // following a continuous Cartesian curve on the surface. Its lag is then
    ...
    if (virtual_progress_ >= wipe_trajectory_.back().time - 1.0e-6) {
      const Eigen::VectorXd final_reference = forceCorrectedReferenceLocked(
        wipe_trajectory_.back().state);
      const double base_error =
        (measured.head<2>() - final_reference.head<2>()).norm();
      const double joint_error =
        (measured.tail<6>() - final_reference.tail<6>()).cwiseAbs().maxCoeff();
      const double ee_error =
        (planner_->framePosition(measured, get_parameter("ee_frame").as_string()) -
        planner_->framePosition(final_reference,
          get_parameter("ee_frame").as_string())).norm();
      const bool settled =
        base_error <= get_parameter(
          "task_completion_base_tolerance").as_double() &&
        joint_error <= get_parameter(
          "task_completion_joint_tolerance").as_double() &&
        ee_error <= get_parameter(
          "task_completion_ee_tolerance").as_double();
      if (!settled) {
        completion_since_.reset();
      } else if (!completion_since_) {
        completion_since_ = current_time;
      } else if (current_time - *completion_since_ >= std::max(
          0.0, get_parameter("task_completion_hold").as_double()))
      {
        virtual_progress_rate_ = 0.0;
        phase_ = Phase::COMPLETED;
        RCLCPP_INFO(
          get_logger(),
          "MuJoCo wiping task completed and settled: base=%.3f m, "
          "joint=%.3f rad, EE=%.3f m",
  ...
  void timerCallback()
  {
    ...
    PlanarTransform control_from_planner;
    if (planner_frame_ != control_frame_) {
      const auto transform = lookupPlanarTransform(control_frame_, planner_frame_);
      if (!transform) {
        return;
      }
      control_from_planner = *transform;
    }
    ...
    std::string current_force_state;
    {
      ...
      for (int i = 0; i < count; ++i) {
        ...
        const Eigen::VectorXd control_state = planner_frame_ == control_frame_ ?
          state : transformWholeBodyState(state, control_from_planner);
        ocs2_msgs::msg::MpcState state_message;
        state_message.value = toFloatVector(control_state);
      ...
      if (live_state_publisher_->get_subscription_count() > 0) {
        sensor_msgs::msg::JointState live_state;
        live_state.header.stamp = now();
        live_state.header.frame_id = planner_frame_;
        live_state.name = {
          "base_x", "base_y", "base_yaw", "joint_1", "joint_2", "joint_3",
          "joint_4", "joint_5", "joint_6"};
        live_state.position.reserve(9);
        for (std::size_t index = 0; index < 9; ++index) {
          live_state.position.push_back(
            static_cast<double>(observation_->state.value[index]));
        }
        live_state_publisher_->publish(live_state);
      }
    }
                             ...
                             Phase current_phase)
  {
    ...
    if (current_phase == Phase::WIPING && current_time - last_tracking_log_ >= 2.0) {
      ...
      const Eigen::Vector3d contact_direction = -planner_->surfaceNormal();
  ...
  void publishWholeBodyContract(const std::vector<Waypoint> & trajectory)
  {
    if (trajectory.empty()) {
      return;
    }
    trajectory_msgs::msg::JointTrajectory message;
    message.header.frame_id = planner_frame_;
    message.header.stamp = now();
    message.joint_names = {
      "base_x", "base_y", "base_yaw", "joint_1", "joint_2", "joint_3",
      "joint_4", "joint_5", "joint_6"};
    const double base_time = trajectory.front().time;
    for (const auto & waypoint : trajectory) {
      trajectory_msgs::msg::JointTrajectoryPoint point;
      point.positions.reserve(waypoint.state.size());
      for (Eigen::Index index = 0; index < waypoint.state.size(); ++index) {
        point.positions.push_back(waypoint.state[index]);
      }
      point.time_from_start = rclcpp::Duration::from_seconds(
        waypoint.time - base_time);
      message.points.push_back(std::move(point));
    }
    whole_body_trajectory_publisher_->publish(message);

    std::vector<std::pair<double, std::string>> intervals;
    const auto contact = std::find_if(
      trajectory.begin(), trajectory.end(),
      [](const Waypoint & waypoint) {return waypoint.in_contact;});
    if (contact == trajectory.end()) {
      intervals.emplace_back(0.0, "PRECONTACT_APPROACH");
    } else {
      const double contact_time = std::max(0.0, contact->time - base_time);
      if (contact_time <= 1.0e-9) {
        intervals.emplace_back(0.0, "TASK_CONSTRAINED");
      } else {
        intervals.emplace_back(0.0, "PRECONTACT_APPROACH");
        intervals.emplace_back(contact_time, "TASK_CONSTRAINED");
      }
    }
    std_msgs::msg::String schedule;
    schedule.data = wbmm_viz::encodePhaseSchedule(intervals);
    phase_schedule_publisher_->publish(schedule);
  }
  ...
  void publishVisualization(const std::vector<Waypoint> & trajectory)
  {
    ...
    if (!trajectory.empty()) {
      ...
      // point on the contact surface and must not be interpreted as a mobile-base goal.
  ...
  std::optional<double> completion_since_;
  ...
  std::atomic_bool collision_fault_latched_{false};
  ...
  rclcpp::Subscription<std_msgs::msg::Bool>::SharedPtr collision_fault_subscription_;
  ...
  rclcpp::Publisher<trajectory_msgs::msg::JointTrajectory>::SharedPtr
    whole_body_trajectory_publisher_;
  rclcpp::Publisher<std_msgs::msg::String>::SharedPtr phase_schedule_publisher_;
  rclcpp::Publisher<sensor_msgs::msg::JointState>::SharedPtr live_state_publisher_;
