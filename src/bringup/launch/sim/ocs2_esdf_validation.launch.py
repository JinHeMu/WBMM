...
def _validate_map_inputs(context):
    required = {
        'esdf_file': LaunchConfiguration('esdf_file').perform(context),
        'map2d_yaml': LaunchConfiguration('map2d_yaml').perform(context),
    }
    if LaunchConfiguration('publish_ply_mesh').perform(context).lower() in (
            '1', 'true', 'yes', 'on'):
        required['ply_file'] = LaunchConfiguration('ply_file').perform(context)

    missing = [
        f'{name}={path!r}' for name, path in required.items()
        if not path or not os.path.isfile(path)
    ]
    if missing:
        raise RuntimeError(
            'OCS2 ESDF validation requires explicit existing map inputs: '
            + ', '.join(missing))
    return []
