...
namespace remani_planner
{
  ...
  void REMANIReplanFSM::init(rclcpp::Node::SharedPtr node)
  {
    ...
    max_consecutive_planning_failures_ = std::max(
        1, declareAndGet<int>(
               node_, "fsm.max_consecutive_planning_failures", 20));
    ...
    // Reaching a goal and deciding to replan are intentionally different
    // decisions. Errors between the acceptance tolerance and these much larger
    // thresholds are left to MPC while REMANI holds the final reference.
    tracking_goal_replan_position_threshold_ =
        std::max(
            tracking_goal_position_tolerance_, declareAndGet<double>(
                node_, "fsm.tracking_goal_replan_position_threshold", 0.40));
    tracking_goal_replan_yaw_threshold_ =
        std::max(
            tracking_goal_yaw_tolerance_, declareAndGet<double>(
                node_, "fsm.tracking_goal_replan_yaw_threshold", 0.70));
    tracking_goal_replan_joint_threshold_ =
        std::max(
            tracking_goal_joint_tolerance_, declareAndGet<double>(
                node_, "fsm.tracking_goal_replan_joint_threshold", 0.45));
    tracking_goal_replan_persistence_ =
        std::max(
            0.0, declareAndGet<double>(
                     node_, "fsm.tracking_goal_replan_persistence", 1.00));
    tracking_error_since_sec_ = -1.0;
    goal_error_since_sec_ = -1.0;
    ...
    RCLCPP_INFO(
        ...
        "goal-replan thresholds=[%.3f m, %.3f rad, %.3f rad] for %.2f s.",
    ...
    RCLCPP_INFO(
        node_->get_logger(),
        "Planning failure limit: %d consecutive failed search cycles; "
        "then abandon the current goal and return to WAIT_TARGET.",
  ...
  void REMANIReplanFSM::execFSMCallback()
  {
     ...
     * 入口条件: 收到新目标
     * 逻辑: 每个周期尝试一次；连续失败达到参数上限后放弃当前目标
     * 出口条件:
     *   ├─ 成功 → EXEC_TRAJ
     *   ├─ 失败(<上限) → GEN_NEW_TRAJ (重试)
     *   └─ 失败(达到上限) → WAIT_TARGET (等待新目标)
     */
    case GEN_NEW_TRAJ:
    {
      ...
      {
        ++replan_fail_time_;
        if (replan_fail_time_ >= max_consecutive_planning_failures_)
        {
          abandonCurrentGoal("initial trajectory search failed repeatedly");
        }
        else
        {
          RCLCPP_WARN(
              node_->get_logger(),
              "[FSM]: initial planning failed (%d/%d); retrying.",
              replan_fail_time_, max_consecutive_planning_failures_);
          changeFSMExecState(GEN_NEW_TRAJ, "FSM");
        }
      }
      break;
     ...
     * 出口条件:
     *   ├─ 成功 → EXEC_TRAJ
     *   ├─ 失败达到参数上限 → WAIT_TARGET (放弃任务)
     *   └─ 失败未到上限 → REPLAN_TRAJ (重试)
     */
    case REPLAN_TRAJ:
    {
      if(planFromLocalTraj(flag_relan_astar_)){
        ...
        t_last_Astar_ = node_->now();
        if(replan_fail_time_ >= max_consecutive_planning_failures_){
          abandonCurrentGoal("local trajectory replanning failed repeatedly");
        }
        else{
    ...
    case EXEC_TRAJ:
    {
      ...
      const bool large_goal_error =
          goal_position_error > tracking_goal_replan_position_threshold_ ||
          goal_yaw_error > tracking_goal_replan_yaw_threshold_ ||
          goal_joint_error > tracking_goal_replan_joint_threshold_;
      ...
      // --- 轨迹播完但实测未达目标 ---
      // 终点参考会继续保持。普通残差由 MPC 收敛；只有超过单独的大误差
      // 阈值并持续一段时间，REMANI 才从实测状态重新规划。该逻辑独立于
      // 运行中 tracking-error 开关，以便保留真正失跟时的恢复能力。
      }else if(trajectory_finished && touch_the_goal && !measured_goal_reached){
        if (!large_goal_error)
        {
          goal_error_since_sec_ = -1.0;
          RCLCPP_INFO_THROTTLE(
              ...
              "Trajectory finished inside the MPC correction envelope "
              "(position=%.3f m, yaw=%.3f rad, max_joint=%.3f rad); "
              "holding the final reference without replanning.",
              goal_position_error, goal_yaw_error, goal_joint_error);
        }
        else
        {
          if (goal_error_since_sec_ < 0.0)
            goal_error_since_sec_ = now_sec;

          const bool persistent =
              now_sec - goal_error_since_sec_ >=
              tracking_goal_replan_persistence_;
          const bool cooldown_elapsed =
              now_sec - last_tracking_replan_sec_ >=
              tracking_error_min_interval_;
          if (persistent && cooldown_elapsed)
          {
            ...
            RCLCPP_WARN(
                node_->get_logger(),
                "Large goal error persisted; replanning from measured state "
                "(position=%.3f m, yaw=%.3f rad, max_joint=%.3f rad).",
                goal_position_error, goal_yaw_error, goal_joint_error);
            if (!planNextWaypoint(end_pt_, end_yaw_))
            {
              RCLCPP_ERROR(
                  node_->get_logger(),
                  "Large-error recovery could not rebuild the global "
                  "trajectory; holding the final reference until the next "
                  "cooldown window.");
            }
            goto force_return;
          }
  ...
  void REMANIReplanFSM::abandonCurrentGoal(const std::string & reason)
  {
    const int failed_cycles = replan_fail_time_;
    const bool had_local_trajectory = have_local_traj_;
    if (had_local_trajectory)
    {
      // Do not leave the controller following a stale trajectory when a local
      // replan is abandoned. ACTION_ABORT makes the bridge/controller hold the
      // measured state while the FSM waits for a fresh goal.
      callEmergencyStop(mm_state_pos_, mm_car_yaw_, mm_car_singul_);
    }
    // A failed goal must be cleared before entering WAIT_TARGET. Otherwise the
    // next timer tick observes the stale target and immediately starts the same
    // impossible search again.
    have_target_ = false;
    have_trigger_ = false;
    have_new_target_ = false;
    have_local_traj_ = false;
    ...
    tracking_error_since_sec_ = -1.0;
    goal_error_since_sec_ = -1.0;
    planner_manager_->resetFailureHistory();
    changeFSMExecState(WAIT_TARGET, "FAIL_LIMIT");

    std_msgs::msg::Bool finished;
    finished.data = false;
    reached_pub_->publish(finished);
    RCLCPP_ERROR(
        node_->get_logger(),
        "Abandoned current goal after %d consecutive failed planning "
        "cycles (%s). REMANI is waiting for a new goal.",
        failed_cycles, reason.c_str());
  }
