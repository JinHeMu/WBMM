...
def generate_launch_description():
    tracer_share = get_package_share_directory('tracer_jaka_description')
    default_urdf = os.path.join(
        tracer_share, 'urdf', 'tracer_jaka_zu5.urdf')
    ...
    tracking_goal_replan_position_threshold = LaunchConfiguration(
        'tracking_goal_replan_position_threshold')
    tracking_goal_replan_yaw_threshold = LaunchConfiguration(
        'tracking_goal_replan_yaw_threshold')
    tracking_goal_replan_joint_threshold = LaunchConfiguration(
        'tracking_goal_replan_joint_threshold')
    tracking_goal_replan_persistence = LaunchConfiguration(
        'tracking_goal_replan_persistence')
    max_consecutive_planning_failures = LaunchConfiguration(
    ...
    manipulator_safe_margin = LaunchConfiguration('manipulator_safe_margin')
    general_safe_margin = LaunchConfiguration('general_safe_margin')
    self_safe_margin = LaunchConfiguration('self_safe_margin')
    ...
    planner = Node(
        ...
        parameters=[
            planner_parameters,
            {
                ...
                'optimization.safe_margin_mani': ParameterValue(
                    manipulator_safe_margin, value_type=float),
                'optimization.safe_margin': ParameterValue(
                    general_safe_margin, value_type=float),
                'optimization.self_safe_margin': ParameterValue(
                    self_safe_margin, value_type=float),
                'mm.mobile_base_max_wheel_omega': ParameterValue(
                    mobile_base_max_wheel_omega, value_type=float),
                'mm.mobile_base_max_wheel_alpha': ParameterValue(
                    mobile_base_max_wheel_alpha, value_type=float),
                'mm.mobile_base_non_singul_vel': ParameterValue(
                    mobile_base_non_singul_vel, value_type=float),
                ...
                'fsm.tracking_goal_replan_position_threshold': ParameterValue(
                    tracking_goal_replan_position_threshold, value_type=float),
                'fsm.tracking_goal_replan_yaw_threshold': ParameterValue(
                    tracking_goal_replan_yaw_threshold, value_type=float),
                'fsm.tracking_goal_replan_joint_threshold': ParameterValue(
                    tracking_goal_replan_joint_threshold, value_type=float),
                'fsm.tracking_goal_replan_persistence': ParameterValue(
                    tracking_goal_replan_persistence, value_type=float),
                'fsm.max_consecutive_planning_failures': ParameterValue(
                    max_consecutive_planning_failures, value_type=int),
            },
        ],
        remappings=[
    ...
    return LaunchDescription([
        ...
        # MuJoCo world and published odometry share the same origin.
        ...
        DeclareLaunchArgument(
            'tracking_goal_replan_position_threshold', default_value='0.40',
            description=(
                'Replan at the end only when base position error exceeds '
                'this large-error threshold; smaller residuals are left to MPC.')),
        DeclareLaunchArgument(
            'tracking_goal_replan_yaw_threshold', default_value='0.70'),
        DeclareLaunchArgument(
            'tracking_goal_replan_joint_threshold', default_value='0.45'),
        DeclareLaunchArgument(
            'tracking_goal_replan_persistence', default_value='1.00',
            description=(
                'Large goal error must persist this long before replanning.')),
        DeclareLaunchArgument(
            'max_consecutive_planning_failures', default_value='20',
            description=(
                'Abandon the current goal and return to WAIT_TARGET after '
                'this many consecutive failed REMANI search cycles.')),
        DeclareLaunchArgument(
        ...
        DeclareLaunchArgument(
            'manipulator_safe_margin', default_value='0.10',
            description=(
                'Clearance added outside every physical URDF manipulator '
                'collision sphere when querying the ESDF.')),
        DeclareLaunchArgument(
            'general_safe_margin', default_value='0.10',
            description='Clearance outside physical base collision spheres.'),
        DeclareLaunchArgument(
            'self_safe_margin', default_value='0.10',
            description='Clearance between non-adjacent robot collision spheres.'),
        DeclareLaunchArgument(
            'mobile_base_max_wheel_omega', default_value='10.0',
            description='Maximum REMANI wheel angular speed in rad/s.'),
        DeclareLaunchArgument(
            'mobile_base_max_wheel_alpha', default_value='20.0',
            description='Maximum REMANI wheel angular acceleration in rad/s^2.'),
        DeclareLaunchArgument(
            'mobile_base_non_singul_vel', default_value='0.05',
            description='Minimum non-singular base speed used by REMANI [m/s].'),
        DeclareLaunchArgument(
