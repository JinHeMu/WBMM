...
namespace remani_planner{
  ...
  int KinoAstar::KinoAstarSearchAndGetSimplePath(
    ...
    if(has_path_){
      ...
      if (flat_trajs_.empty()) {
        // The A* search can complete with a single Reeds-Shepp shot from the
        // start node (free space / nearby goals).  When the trapezoid time
        // allocation then produces zero flat pieces -- e.g. a zero-length
        // segment hits the early return in getTrajsWithTime -- the whole-body
        // sampler below would index car_state_list_ (empty) out of bounds and
        // segfault.  Abort this search as NO_PATH; the FSM retries.
        RCLCPP_WARN(node_->get_logger(),
                    "KinoAstar: A* path produced no timed base pieces "
                    "(path_nodes=%zu, sample_trajs=%zu, shot=%d, times=%zu); "
                    "aborting search (NO_PATH)",
                    path_nodes_.size(), sample_trajs_.size(),
                    is_shot_succ_ ? 1 : 0, shot_timeList.size());
        has_path_ = false;
  ...
  void KinoAstar::getTrajsWithTime(){
    // A shot-only path can leave fewer than two base samples; the singul
    // segmentation below indexes sample_trajs_[1]. Leave flat_trajs_ empty so
    // KinoAstarSearchAndGetSimplePath falls back to the straight-line + RRT
    // path instead of reading out of bounds.
    if (sample_trajs_.size() < 2) {
      RCLCPP_WARN(node_->get_logger(),
                  "KinoAstar: too few base samples (%zu) for time allocation",
                  sample_trajs_.size());
      return;
    }
