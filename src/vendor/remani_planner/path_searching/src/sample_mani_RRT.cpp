...
namespace mani_sample{
  ...
  bool SampleMani::sampleManiSearch(const bool astar_succ,
    ...
    // Defensive guard: the sampler indexes car_state_list_[node->index] in
    // initNode(), so an empty base path would be an out-of-bounds read.
    if (car_state_list_.empty()) {
      RCLCPP_WARN(node_->get_logger(),
                  "SampleMani: empty base car state list; aborting whole-body "
                  "sampling instead of crashing");
      return false;
    }
