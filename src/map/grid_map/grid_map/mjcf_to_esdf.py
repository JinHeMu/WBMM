"""Generate a REMANI-compatible static ESDF from a MuJoCo scene."""
...
def is_static_body(model, body_id):
    """Return whether a body and all of its ancestors have no joints.
...
def voxelize_oriented_box(occ, origin, voxel_size, center, R, half_size):
    """Voxelize an oriented MuJoCo box into the occupancy grid.
...
def build_esdf_from_mjcf(
    ...
    """Voxelize static scene geometry and save its signed distance field."""
    if not frame_id or frame_id.startswith("/"):
        raise ValueError("frame_id must be a non-empty ROS frame without '/'")
    ...
    for gid in range(model.ngeom):
        ...
        if geom_type == BOX:
            ...
            print(
                f"[WARN] skip unsupported geom type: {name}, "
                f"type={geom_type}"
            )

    # ESDF：自由空间为正，障碍内部为负。没有任何静态障碍时不要对全 True
    # 数组调用 EDT；其中不存在背景零点，边界距离不代表真实障碍距离。空环境
    # 直接写成一个统一的大正距离，确保 REMANI 将整个归档范围视为自由空间。
    if np.any(occ):
        ...
        esdf = np.full(occ.shape, 100.0, dtype=np.float32)
...
def main():
    """Parse command-line arguments and generate the requested ESDF archive."""
    ...
    parser.add_argument(
        "--bounds-min", type=float, nargs=3, default=(-1.0, -3.0, 0.0)
    )
    parser.add_argument(
        "--bounds-max", type=float, nargs=3, default=(5.0, 3.0, 1.8)
    )
    parser.add_argument("--include-floor", action="store_true")
    parser.add_argument(
        "--frame-id",
        default="odom",
        help="ROS frame in which the generated ESDF coordinates are expressed.",
    )
