...
class NvbloxMapExporter(Node):
    ...
    def wait_until_input_drained(self, timings_output):
        ...
        while rclpy.ok() and time.monotonic() < deadline:
            ...
            # The bag's final depth frame can legitimately wait forever for
            # TF at its own stamp (nothing is recorded after it) and the sim
            # clock stops with the player, so allow a small stuck tail.
