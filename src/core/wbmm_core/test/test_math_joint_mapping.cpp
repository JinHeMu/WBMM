#include "wbmm_core/math/conversions.hpp"
...
TEST(JointMappingTest, EigenStateRoundTripPreservesExplicitOrder)
{
  const auto original = differentialDriveStateVector(reorderedState(), kExpectedOrder);
  ASSERT_TRUE(original.ok());
  const auto domain_state = differentialDriveStateFromVector(
    original.value(), {"map", validStamp()}, kExpectedOrder);
  ASSERT_TRUE(domain_state.ok());
  EXPECT_EQ(domain_state.value().base_model, core::BaseModel::kDifferentialDrive);
  EXPECT_EQ(domain_state.value().joints.names, kExpectedOrder);

  const auto round_trip = differentialDriveStateVector(domain_state.value(), kExpectedOrder);
  ASSERT_TRUE(round_trip.ok());
  EXPECT_TRUE(round_trip.value().isApprox(original.value()));
}

TEST(JointMappingTest, DuplicateActualNamesFailClosedInMapping)
{
  auto state = reorderedState();
  state.joints.names[0] = state.joints.names[1];
  const auto result = differentialDriveStateVector(state, kExpectedOrder);
  EXPECT_FALSE(result.ok());
  EXPECT_EQ(result.status().code(), core::ErrorCode::kDuplicateJointName);
}
...
TEST(JointMappingTest, WrongEigenStateDimensionFailsClosed)
{
  const Eigen::VectorXd vector = Eigen::VectorXd::Zero(8);
  const auto result = differentialDriveStateFromVector(
    vector, {"map", validStamp()}, kExpectedOrder);
  EXPECT_FALSE(result.ok());
  EXPECT_EQ(result.status().code(), core::ErrorCode::kInvalidDimension);
}

TEST(JointMappingTest, NonFiniteStateFailsClosed)
{
  Eigen::VectorXd vector = Eigen::VectorXd::Zero(9);
  vector[3] = std::numeric_limits<double>::quiet_NaN();
  const auto result = differentialDriveStateFromVector(
    vector, {"map", validStamp()}, kExpectedOrder);
  EXPECT_FALSE(result.ok());
  EXPECT_EQ(result.status().code(), core::ErrorCode::kNonFiniteValue);
