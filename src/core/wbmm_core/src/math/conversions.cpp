...
core::Result<core::WholeBodyState> differentialDriveStateFromVector(
  const Eigen::VectorXd & state,
  const core::Header & header,
  const std::vector<std::string> & joint_order)
{
  if (const auto status = core::validateHeader(header); !status.ok()) {
    return core::Result<core::WholeBodyState>::Failure(status);
  }
  if (const auto status = core::validateJointOrder(joint_order, joint_order); !status.ok()) {
    return core::Result<core::WholeBodyState>::Failure(status);
  }
  const auto expected_size = static_cast<Eigen::Index>(
    core::DifferentialDriveContract::stateDimension(joint_order.size()));
  if (state.size() != expected_size) {
    std::ostringstream stream;
    stream << "differential-drive state has dimension " << state.size()
           << ", expected " << expected_size;
    return core::Result<core::WholeBodyState>::Failure(
      {core::ErrorCode::kInvalidDimension, stream.str()});
  }
  if (!state.allFinite()) {
    return core::Result<core::WholeBodyState>::Failure(
      {core::ErrorCode::kNonFiniteValue,
        "differential-drive state contains a non-finite value"});
  }

  core::WholeBodyState result;
  result.header = header;
  result.base_model = core::BaseModel::kDifferentialDrive;
  result.base.x_m = state[0];
  result.base.y_m = state[1];
  result.base.yaw_rad = state[2];
  result.base.linear_velocity_mps = 0.0;
  result.base.lateral_velocity_mps = 0.0;
  result.base.yaw_rate_radps = 0.0;
  result.joints.names = joint_order;
  result.joints.positions_rad.resize(joint_order.size());
  for (std::size_t index = 0; index < joint_order.size(); ++index) {
    result.joints.positions_rad[index] = state[static_cast<Eigen::Index>(index + 3)];
  }
  return core::Result<core::WholeBodyState>::Success(std::move(result));
}
