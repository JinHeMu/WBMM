...
public:
      ...
      : Node("remani_to_ocs2_reference_bridge")
  {
    ...
    referenceOwnerService_ = declare_parameter<std::string>(
        "reference_owner_service", "/remani_bridge/set_reference_enabled");
    ...
    referenceOwnerServiceHandle_ = create_service<std_srvs::srv::SetBool>(
        referenceOwnerService_,
        std::bind(
            &RemaniToOcs2ReferenceBridge::setReferenceEnabled,
  ...
  void init()
  {
    setTargetPublisherEnabled(true);
  }

private:
  void setTargetPublisherEnabled(bool enabled)
  {
  ...
  void setReferenceEnabled(
      const std::shared_ptr<std_srvs::srv::SetBool::Request> request,
      std::shared_ptr<std_srvs::srv::SetBool::Response> response)
  {
    setTargetPublisherEnabled(request->data);
    response->success = true;
    response->message = request->data
        ? "REMANI bridge acquired MPC reference ownership"
        : "REMANI bridge released MPC reference ownership";
  ...
  {
    if (!referenceEnabled_ || !targetPublisher_)
    {
      return;  // init() 尚未调用
    }
