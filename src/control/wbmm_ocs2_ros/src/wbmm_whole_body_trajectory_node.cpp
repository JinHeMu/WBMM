// =============================================================================
//  wbmm_whole_body_trajectory_node.cpp
//
//  全身轨迹目标发布节点。
//
...
class WbmmWholeBodyTrajectoryNode : public rclcpp::Node {
 public:
  WbmmWholeBodyTrajectoryNode() : Node("wbmm_whole_body_trajectory_node") {
    // ── 基础参数 ────────────────────────────────────────────
  ...
  void init() {
    ...
    publishSrv_ = create_service<std_srvs::srv::Trigger>(
        ...
        std::bind(&WbmmWholeBodyTrajectoryNode::onPublishService, this,
    ...
    if (autoPublish_) {
      autoTimer_ = create_wall_timer(
          ...
          std::bind(&WbmmWholeBodyTrajectoryNode::onAutoTimer, this));
    }
...
int main(int argc, char** argv) {
  ...
  auto node = std::make_shared<WbmmWholeBodyTrajectoryNode>();
  node->init();
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
