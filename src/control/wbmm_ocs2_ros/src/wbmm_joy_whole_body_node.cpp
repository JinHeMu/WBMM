...
class WbmmJoyWholeBodyNode : public rclcpp::Node {
 public:
  WbmmJoyWholeBodyNode() : Node("wbmm_joy_whole_body_node") {
    // ── 基础参数 ──────────────────────────────────────────────
    ...
    joySub_ = create_subscription<sensor_msgs::msg::Joy>(
        ...
        std::bind(&WbmmJoyWholeBodyNode::onJoy, this,
                  std::placeholders::_1));
  ...
  void init() {
    ...
    timer_ = create_wall_timer(
        period_ns,
        std::bind(&WbmmJoyWholeBodyNode::onTimer, this));
...
int main(int argc, char** argv) {
  ...
  auto node = std::make_shared<WbmmJoyWholeBodyNode>();
  node->init();
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
