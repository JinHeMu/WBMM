...
class WbmmTargetNode : public rclcpp::Node {
 public:
  WbmmTargetNode() : Node("wbmm_target_node") {
  ...
  void init() {
    ...
    menuHandler_.insert(
        "Send target",
        std::bind(&WbmmTargetNode::onMenuSend, this, std::placeholders::_1));

    server_->insert(marker);
    server_->setCallback(
        marker.name,
        std::bind(&WbmmTargetNode::onMarkerFeedback, this, std::placeholders::_1));
...
int main(int argc, char** argv) {
  ...
  auto node = std::make_shared<WbmmTargetNode>();
  node->init();
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
