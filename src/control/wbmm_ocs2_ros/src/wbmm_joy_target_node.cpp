...
class WbmmJoyTargetNode : public rclcpp::Node {
 public:
  WbmmJoyTargetNode() : Node("wbmm_joy_target_node") {
    // ---- 基础参数 ----
    ...
    joy_sub_ = create_subscription<sensor_msgs::msg::Joy>(
        ...
        std::bind(&WbmmJoyTargetNode::onJoy, this, std::placeholders::_1));
  ...
  void init() {
    ...
    timer_ = create_wall_timer(
        period_ns,
        std::bind(&WbmmJoyTargetNode::onTimer, this));
...
int main(int argc, char** argv) {
  ...
  auto node = std::make_shared<WbmmJoyTargetNode>();
  node->init();
