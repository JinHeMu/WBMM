...
#include <wbmm_ocs2/AccessHelperFunctions.h>
...
namespace wbmm
{
    ...
    using namespace wbmm_ocs2;

    WbmmVisualization::WbmmVisualization(
        const rclcpp::Node::SharedPtr& node,
        const WbmmInterface& interface,
        std::string worldFrame,
        bool enableSelfCollision)
        : node_(node),
          pinocchioInterface_(interface.getPinocchioInterface()),
          modelInfo_(interface.getWbmmModelInfo()),
    ...
    void WbmmVisualization::setupPublishers()
    {
        ...
        obstacleMarkerPub_ = node_->create_publisher<visualization_msgs::msg::MarkerArray>(
            "/wbmm/environmentObstacles",
            rclcpp::QoS(rclcpp::KeepLast(1)).reliable().transient_local());
    }

    void WbmmVisualization::setupSelfCollision(bool enable)
    {
        if (!enable) { return; }
        ...
        try {
            ...
            RCLCPP_WARN(node_->get_logger(),
                        "[WbmmVisualization] 读取 taskFile/urdfFile 参数失败: %s; "
                        "跳过自碰撞可视化。", e.what());
            return;
        }
        ...
        try {
            PinocchioInterface pinocchioInterface(
                createWbmmPinocchioInterface(urdfFile, removeJointNames_));
            ...
            RCLCPP_INFO(node_->get_logger(),
                        "[WbmmVisualization] 自碰撞距离可视化已启用 (activationDistance=%.3f)。",
                        activationDistance);
        } catch (const std::exception& e) {
            RCLCPP_WARN(node_->get_logger(),
                        "[WbmmVisualization] 初始化自碰撞可视化失败: %s; 已跳过。",
                        e.what());
    ...
    // 和 wbmm_ocs2::WbmmInterface::loadInitialObstacles 用同一份配置。
    // =======================================================================
    void WbmmVisualization::publishObstacles()
    {
        ...
        try { boost::property_tree::read_info(taskFile, pt); }
            ...
            RCLCPP_WARN(node_->get_logger(),
                        "[WbmmVisualization] 解析 task 文件失败: %s", e.what());
            return;
        }
        ...
        if (!envActivate) {
            RCLCPP_INFO(node_->get_logger(),
                        "[WbmmVisualization] environmentCollision.activate=false, 无障碍物可视化。");
            return;
        }
        ...
        RCLCPP_INFO(node_->get_logger(),
                    "[WbmmVisualization] 已发布 %d 个环境障碍物 marker 到 "
                    "/wbmm/environmentObstacles (frame=%s)。",
                    count, worldFrame_.c_str());
    }

    void WbmmVisualization::update(const SystemObservation& observation,
    ...
    void WbmmVisualization::publishOptimizedTrajectory(const rclcpp::Time& stamp,
                                                            const PrimalSolution& policy)
    {
    ...
    void WbmmVisualization::publishReferenceTrajectory(
        const rclcpp::Time& stamp, const TargetTrajectories& target)
    {
        ...
        else
        {
            RCLCPP_WARN_THROTTLE(node_->get_logger(), *node_->get_clock(), 5000,
                                 "[WbmmVisualization] 参考 target 维度 %d 无法识别, 跳过。",
                                 refDim);
            return;
        }
