...
{
  using wbmm::robot::PinocchioRobotModel;
  using wbmm::robot::WholeBodyKinematics;
  using wbmm::robot::toCoreState;
  using wbmm::robot::toEigenState;
  using wbmm::robot::toCoreWrench;
  using wbmm::robot::toEigenWrench;
  using wbmm::robot::toMpcTargetTrajectories;
  using wbmm::robot::makeZeroWholeBodyInput;
  using wbmm::robot::wholeBodyStateFromMpcObservation;
  using wbmm::robot::wrenchFromRos;
  ...
  public:
        ...
        : Node("whole_body_force_control")
    {
      ...
      max_velocity_defaults << max_velocity, max_velocity, max_velocity, 0.15, 0.15, 0.15;
      ...
      const double legacy_desired = control_mode_ == "constant_force" ? configured_desired_force : 0.0;
  ...
  private:
    ...
    void update()
    {
      ...
      else
      {
        ...
        primary_offset = force_follow ? force_follower_->update(measured_force_, dt) : admittance_->update(measured_force_, dt);
        primary_force = force_follow ? force_follower_->measuredForce() : admittance_->measuredForce();
        ...
        const Vector6d &correction)
    {
      ...
      const Vector6d velocity = cartesian_mode_ ? cartesian_controller_->velocity() : Vector6d::Zero();
      for (Eigen::Index i = 0; i < 6; ++i)
      {
