 ...
 *
 * 全身轨迹跟踪 ocs2::StateCost:
 *
 *   L(x, t) = 0.5 * (x - x_d(t))^T Q (x - x_d(t))
 *
 *   x_d(t) 直接来自 OCS2 传进来的 ocs2::TargetTrajectories (即 ocs2::ReferenceManager 里
...
class WholeBodyTrajectoryCost final : public ocs2::StateCost {
 public:
  /**
   * @param Q             stateDim x stateDim 的权重矩阵
   * @param yawIndex      底盘 yaw 在 state 中的下标; 无 yaw 时传 -1
   * @param fallbackState 当 ocs2::TargetTrajectories 为空时使用的参考 (一般是 initialState)
   */
  WholeBodyTrajectoryCost(ocs2::matrix_t Q, int yawIndex, ocs2::vector_t fallbackState);
 ...
 private:
  ...
  /** 把角度 wrap 到 [-pi, pi] */
  static ocs2::scalar_t wrapToPi(ocs2::scalar_t angle);
   ...
   */
  ocs2::vector_t getDesiredState(ocs2::scalar_t time,
                           const ocs2::TargetTrajectories& targetTrajectories) const;

  ocs2::matrix_t Q_;
  int      yawIndex_{-1};
  ocs2::vector_t fallbackState_;
};
