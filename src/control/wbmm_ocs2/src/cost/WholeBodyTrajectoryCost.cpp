...
WholeBodyTrajectoryCost::WholeBodyTrajectoryCost(
  matrix_t Q, int yawIndex,
  ...
  fallbackState_(std::move(fallbackState))
{
  ...
  if (!Q_.allFinite() || !fallbackState_.allFinite()) {
    throw std::runtime_error(
            "[WholeBodyTrajectoryCost] Q and fallbackState must be finite!");
  }
  ...
  scalar_t time, const TargetTrajectories & targetTrajectories) const
{
  ...
  // 2) 非法 reference 采用初始状态保持，禁止静默关闭跟踪代价。
  const bool invalidReference = ts.size() != xs.size() ||
    !std::all_of(
    ts.begin(), ts.end(), [](scalar_t value) {
      return std::isfinite(value);
    }) ||
    std::adjacent_find(
    ts.begin(), ts.end(),
    [](scalar_t first, scalar_t second) {return second <= first;}) != ts.end() ||
    !std::all_of(
    xs.begin(), xs.end(), [stateDim](const vector_t & state) {
      return state.size() == stateDim && state.allFinite();
    });
  if (invalidReference) {
    static bool warned = false;
    if (!warned) {
      warned = true;
      std::cerr << "[WholeBodyTrajectoryCost] WARNING: invalid whole-body "
        "TargetTrajectories; using fallback hold state.\n";
    }
    return fallbackState_;
  }
