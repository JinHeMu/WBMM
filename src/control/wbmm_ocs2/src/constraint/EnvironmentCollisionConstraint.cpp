...
// 迁移期文件级可见性，见 WbmmInterface.cpp 说明。
...
WbmmEnvironmentCollisionConstraint::WbmmEnvironmentCollisionConstraint(
    const PinocchioStateInputMapping<scalar_t>& mapping,
    std::shared_ptr<EnvironmentGeometryInterface> envGeomInterface,
    scalar_t minimumDistance)
...
WbmmEnvironmentCollisionConstraint*
WbmmEnvironmentCollisionConstraint::clone() const {
    return new WbmmEnvironmentCollisionConstraint(*this);
}

const PinocchioInterface& WbmmEnvironmentCollisionConstraint::getPinocchioInterface(
    const PreComputation& preComputation) const
{
